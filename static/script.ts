...
// -- SVG Elements
const dotContainer = document.getElementById('dot-container') as SVGElement;
const xAxisTicks = document.getElementById('x-axis-ticks') as SVGElement;
const yAxisTicks = document.getElementById('y-axis-ticks') as SVGElement;
...
async function saveResultsToDB(results: ServerResponse[]): Promise<void> {
    await setItem(KEY_FOR_TABLE_HISTORY, results);
}

async function loadResultsFromDB(): Promise<ServerResponse[]> {
    const savedResults = await getItem<ServerResponse[]>(KEY_FOR_TABLE_HISTORY);
    return savedResults || [];
}

async function updateResultsWithNewRow(data: ServerResponse): Promise<void> {
    const resultsHistory = await loadResultsFromDB();
