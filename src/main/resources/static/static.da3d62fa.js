...
})(
    {
        belta: [
            function (require, module, exports, __globalThis) {
                ...
                var HMR_HOST = null;
                var HMR_PORT = null;
                var HMR_SERVER_PORT = 61119;
                var HMR_SECURE = false;
                var HMR_ENV_HASH = '439701173a9199ea';
                var HMR_USE_SSE = false;
                module.bundle.HMR_BUNDLE_ID = 'da12ca55da3d62fa';
                ('use strict');
...
${frame.code}`;
        ...
        '2wcqF': [
            function (require, module, exports, __globalThis) {
                ...
                document.addEventListener('DOMContentLoaded', async () => {
                    ...
                    const resultsHistory = await loadResultsFromDB();
                ...
                function validateAllFields() {
                    if (hiddenXInput.value === '') return 'Please select an X value.';
                    const yValueStr = yInput.value.trim().replace(',', '.');
                    if (yValueStr === '') return 'Please enter a Y value.';
                    if (isNaN(parseFloat(yValueStr))) return 'The Y value must be a number.';
                    const yNum = parseFloat(yValueStr);
                    if (yNum < -5 || yNum > 5)
                        return 'The Y value must be in the range (-5 ... 5).';
                    if (rSelect.value === '') return 'Please select an R value.';
                    return null;
                }
                async function sendDataToServer() {
                    ...
                    try {
                        ...
                        await updateResultsWithNewRow(data);
                    } catch (error) {
    ...
    `;
                ...
                async function saveResultsToDB(results) {
                    await (0, _db.setItem)(KEY_FOR_TABLE_HISTORY, results);
                }
                async function loadResultsFromDB() {
                    const savedResults = await (0, _db.getItem)(KEY_FOR_TABLE_HISTORY);
                    return savedResults || [];
                }
                async function updateResultsWithNewRow(data) {
                    const resultsHistory = await loadResultsFromDB();
                    resultsHistory.unshift(data);
                    await saveResultsToDB(resultsHistory);
                }
                function updateGraphLabels(r) {
                    xAxisTicks.innerHTML = '';
                    yAxisTicks.innerHTML = '';
                    const createTick = (axis, value, label) => {
                    ...
                    if (!r || isNaN(r)) {
                        ...
                        labels.forEach((label, i) => {
                            createTick('x', values[i], label);
                            createTick('y', values[i], label);
                        });
                    } else {
        ...
        kppXW: [
            function (require, module, exports, __globalThis) {
                ...
                async function initDB() {
                    if (db) return db;
                    db = await (0, _idb.openDB)(DB_NAME, DB_VERSION, {
                        upgrade(db) {
            ...
            { idb: '258QC', '@parcel/transformer-js/src/esmodule-helpers.js': 'jnFvT' },
        ],
        '258QC': [
            function (require, module, exports, __globalThis) {
                var parcelHelpers = require('@parcel/transformer-js/src/esmodule-helpers.js');
...
//# sourceMappingURL=static.da3d62fa.js.map
